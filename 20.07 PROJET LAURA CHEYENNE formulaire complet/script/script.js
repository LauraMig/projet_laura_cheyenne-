...
const baseUrl = 'https://api.gbif.org/v1/species/search?q=';
const cityDOM = document.querySelector('.weather__city');
const validateDOM = document.querySelector('.weather__validate');
const forecastDOM = document.querySelector('.weather__forecast');
validateDOM.addEventListener('click', (event) => getForecast(event, cityDOM.value));
function getForecast(event, animalName) {
    event.preventDefault();
    if (animalName !== '') {
        const xhr = new XMLHttpRequest();
        const method = 'GET';
        const endpoint = baseUrl + animalName;
        xhr.open(method, endpoint, true);
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                const userData = JSON.parse(xhr.responseText);
                if (userData.results.length > 0) {
                    console.log(userData.results[8]);
                    const animalData = userData.results[0];
                    let forecastReport = `
            <ul>
              <li><strong>${animalName}</strong></li>
              <li>Espéce : ${animalData.scientificName || 'non renseigné'}</li>
              <li>Famille : ${animalData.family || 'non renseigné'}</li>
              <li>Ordre : ${animalData.order || 'non renseigné'}</li>
              <li>Genre : ${animalData.genus || 'non renseigné'}</li>
              <li>Statut taxonomique : ${animalData.taxonomicStatus || 'non renseigné'}</li>
              <li>Authortship : ${animalData.authorship || 'non renseigné'}</li>
              <li>Kingdom : ${animalData.kingdom || 'non renseigné'}</li>
              <li>Phylum : ${animalData.phylum || 'non renseigné'}</li>
              <li>Rank : ${animalData.rank || 'non renseigné'}</li>
              <li>Class : ${animalData.class || 'non renseigné'}</li>
              </ul>
            `;
                    forecastDOM.innerHTML = forecastReport;
                } else {
                    forecastDOM.innerHTML = '<p>Aucun résultat trouvé pour cette espèce.</p>';
                }
            }
        };
        xhr.send();
